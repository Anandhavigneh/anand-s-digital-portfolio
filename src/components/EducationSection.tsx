...
const EducationSection = () => {
  const education = [{
    ...
    achievements: ["Developed a full-stack web application for campus resource management", "Participated in university coding competitions and hackathons", "Collaborated on research project exploring Crypto & blockchain applications"],
    ...
    achievements: ["Python Application Development", "Database Design (MySQL)", "Web Development Projects", "Problem Solving & Algorithms", "Crypto & Blockchain Basics"],
  ...
  return <section id="education" className="py-20 md:py-32 relative bg-card/30">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          {education.map((edu, index) => <div key={edu.institution} className="group p-6 md:p-8 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-300">
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  ...
                  {/* Achievements */}
                  {edu.achievements.length > 0 && <div className="mb-4">
                      <div className="flex items-center gap-2 text-foreground font-semibold mb-2">
                        <Award className="w-4 h-4 text-primary" />
                        <span>Achievements</span>
                      </div>
                      <ul className="space-y-1">
                        {edu.achievements.map((achievement, i) => <li key={i} className="text-sm text-muted-foreground flex items-start gap-2">
                  ...
                  {edu.courses.length > 0 && <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {edu.courses.map((course, i) => <span key={i} className="px-3 py-1 text-xs rounded-full bg-primary/10 text-primary border border-primary/20">
                            {course}
                          </span>)}
                      </div>
                    </div>}
